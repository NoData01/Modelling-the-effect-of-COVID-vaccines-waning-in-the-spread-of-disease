...
sigma = 0.11;       %vaccine inefficiency
lambda = 1/7;     %average latet time for exposed to infected
kappa_s = 0.013;    %death rate of susceptible people(assume)
...
dydt = [-(beta*y(1)*y(3))/N-alpha*y(1)-(sigma*beta*y(1)*y(3))/N;   %S
        (beta*y(1)*y(3))/N+(sigma*beta*y(6)*y(8))/N-lambda*y(2);   %Es
        lambda*y(2)-kappa_s*y(3)*tau-(1-kappa_s)*y(3)*mu;  %Is
        (1-kappa_s)*y(3)*mu+(1-kappa_v)*y(8)*mu;           %R
        kappa_s*y(3)*tau+kappa_v*y(8)*tau;        %D
        alpha*y(1)-(sigma*beta*y(6)*y(8))/N-(sigma*beta*y(6)*y(8))/N; %V
        (sigma*beta*y(1)*y(3))/N+(sigma*beta*y(6)*y(8))/N-lambda*y(7); %Ev
